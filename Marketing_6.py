...
class Login(QWidget):
    ...
    # 로그인 UI
    def loginUI(self):
        # Ycode_L와 NAME을 입력할 Label과 Line을 생성
    ...
    def pushButtonCliked(self):
        ...
        self.Ycode_L = self.lineEdit1.text()
        self.NAME_L = self.lineEdit2.text()
        ...
        # Ycode_L와 이름이 일치할 경우 DF 생성, 아닐경우 빈 DF 생성으로 로그인 성공과 실패를 결정
        self.login_info = self.login_db.loc[(self.login_db['Ycode'] == self.Ycode_L) & (self.login_db['Name'] == self.NAME_L)]
        ...
        if self.login_info.empty:
            ...
            self.close()
            # 다음 위젯을 실행
            return (self.Ycode_L, self.NAME_L)
...
class tab_widget(QWidget):
    ...
    def code_c_search(self):
        ...
        # GropBox 크기 조절
        self.c_code_search.setMaximumSize(430, 900)
        ...
        # TableWidget 설정
        self.c_code_table.resize(300, 900)
        # Table 짝수번째 색 변화
