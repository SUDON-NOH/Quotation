...
class tab_widget(QWidget):
    ...
    def exeUI(self):
        ...
        self.tabs.addTab(QTextEdit(self.tab2), 'Progress')
    ...
    def code_c_search(self):
        ...
        self.Quotation = QGroupBox('Quotation')

        self.tab1.layout.addWidget(self.c_code_search, 0, 0)
        self.tab1.layout.addWidget(self.Quotation, 0, 1)

        self.code_label = QLabel('검 색:')
        self.code_line = QLineEdit()

        # c_code_search 의 GroupBox 에 대한 정렬코드
        Hbox = QHBoxLayout()
        Hbox.addWidget(self.code_label)
        Hbox.addWidget(self.code_line)
        self.c_code_search.setLayout(Hbox)
